...
//SOLICITUD PARA TRAERME MIS 100 VIDEOJUEGOSss
//A LA API
const infoApi = async() => {
    let url = `https://api.rawg.io/api/games?key=${API_KEY}` //flag 1
    let videojuegos = []
    try {
...
const nameApi = async (name) => {
    const infoSearch = await axios.get(`https://api.rawg.io/api/games?search=${name}&key=${API_KEY}`) //flag 2
...
const idApi = async (id) => {
    try {
        const rtaApi = await axios.get(`https://api.rawg.io/api/games/${id}?key=${API_KEY}`) //flag 4
        if(rtaApi) {
